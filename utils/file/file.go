...
func GetFiles(taskInfo *mesos.TaskInfo) ([]string, error) {
	...
	var files []string
	for _, file := range strings.Split(filelist, FileDelimiter) {
...
func GetPluginOrder(taskInfo *mesos.TaskInfo) ([]string, error) {
	...
	var plugins []string
	for _, plugin := range strings.Split(pluginList, FileDelimiter) {
...
func DumpPluginModifiedComposeFiles(ctx context.Context, plugin, funcName string, pluginOrder int) {
	...
	for file := range filesMap {
		content, _ := yaml.Marshal(filesMap[file])
		fParts := strings.Split(file.(string), PathDelimiter)
		if len(fParts) < 2 {
...
func SearchInArray(array []interface{}, key string) string {
	for _, e := range array {
		if s := strings.Split(e.(string), MapDelimiter); len(s) > 1 && s[0] == key {
			return s[1]
		}
	}
	return ""
...
func FolderPath(filenames []string) []string {
	...
	for i, filename := range filenames {
		if !strings.Contains(filename, config.GetAppFolder()) {
			filenames[i] = strings.TrimSpace(folder) + PathDelimiter + filename
		}
	}
...
// into folder
func GenerateAppFolder(composeFiles []string) error {
	folder := config.GetAppFolder()
	if folder == "" {
